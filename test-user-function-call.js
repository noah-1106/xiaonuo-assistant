...
// 用户在最新消息中提供的内容
const userLatestContent = '[FUNCTION_CALL]createTask({"title":"修改控制体型记录状态","subtasks":[{"id":"subtask-1","title":"更新记录状态为已完成","description":"将记录ID为6984afb72004d003a0311f8d的待办事项状态从pending改为completed","functionCall":{"name":"updateRecord","arguments":{"recordId":"6984afb72004d003a0311f8d","status":"completed"}}},"executionMode":"auto"})[/FUNCTION_CALL] ';

console.log('=== 测试用户提供的函数调用格式 ===');
console.log('用户最新提供的内容:', userLatestContent);
...
// 执行测试
try {
  const { functionName, jsonStr } = extractJson(userLatestContent);
...
try {
  const { functionName, jsonStr } = extractJson(correctFormat);
  const isValid = validateJson(jsonStr);
  
  console.log('\n=== 修复格式测试结果 ===');
  if (isValid) {
    console.log('✅ 修复后的格式正确');
  } else {
    console.log('❌ 修复后的格式仍然有问题');
  }
} catch (error) {
  console.error('修复格式测试失败:', error);
}
