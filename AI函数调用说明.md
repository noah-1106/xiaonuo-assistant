...
```mixin-react
return (<Tabs>
<Tabs.TabPane title="Python - Ark SDK" key="r7ehONGMxJ"><RenderMd content={`**方舟基础SDK**
\`\`\`Python
from volcenginesdkarkruntime import Ark
...
<Tabs.TabPane title="Python - Arkiteck SDK" key="VzCAit5bc0"><RenderMd content={`**使用方舟智能体SDK Arkitect**
\`\`\`Python
...
`}></RenderMd></Tabs.TabPane>
<Tabs.TabPane title="Java" key="K2WnennBU6"><RenderMd content={`\`\`\`Java
...
`}></RenderMd></Tabs.TabPane>
<Tabs.TabPane title="Golang" key="BVzNtKFr1A"><RenderMd content={`\`\`\`Go
package main
...
3. 常规调优手段：
   ...
   * 对模型进行sft（建议至少50条数据，模型越多、参数越多、情况越多，则所需要的数据越多），见[效果优化](/docs/82379/1262342#f118e5a2)。
...
启用严格模式需要满足以下前提条件（参见[结构化输出(beta)](/docs/82379/1568221)）：
...
```mixin-react
return (<Tabs>
<Tabs.TabPane title="启用 Strict 模式" key="LynXvR12Gq"><RenderMd content={`\`\`\`JSON
{
  "type": "function",
  "function": {
    "name": "get_current_weather",
...
`}></RenderMd></Tabs.TabPane>
<Tabs.TabPane title="不启用 Strict 模式" key="YVKdLWsKDi"><RenderMd content={`\`\`\`JSON
{
  "type": "function",
  "function": {
    "name": "get_current_weather",
...
```mixin-react
return (<Tabs>
<Tabs.TabPane title="Golang" key="tB97HUKc5p"><RenderMd content={`\`\`\`Go
package main
...
`}></RenderMd></Tabs.TabPane>
<Tabs.TabPane title="Python" key="j2pbsJPcWC"><RenderMd content={`\`\`\`Python
from volcenginesdkarkruntime import Ark
import time
client = Ark(
    base_url="\\${BASE_URL}",
...
`}></RenderMd></Tabs.TabPane>
<Tabs.TabPane title="Java" key="gdv7hxLV3z"><RenderMd content={`\`\`\`Java
